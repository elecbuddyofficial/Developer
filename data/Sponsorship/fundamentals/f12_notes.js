...
<div class="note-doc">
  ...
  <p class="n-p">The reason this shape is necessary goes back to F11. A motor starting DOL draws five to seven times full load current for a few seconds while it accelerates, a normal and harmless event. A relay with a flat, fast trip point would trip on every single start. The inverse curve solves this: at five to seven times FLC the allowed time is several seconds, long enough for a healthy motor to finish accelerating before the relay would have tripped. If that same high current persists instead, because the rotor is jammed and never accelerates, the relay still trips, just later than it would for a short circuit. Time is the discriminator between starting and stalled, without the relay ever needing to know which one is actually happening.</p>

  <div class="n-crit"><span class="icon">💡</span><div class="body"><strong>Say this in the interview.</strong> The overload relay has to ignore the starting surge but still catch a locked rotor, and both look like "high current" to the relay. The only way to tell them apart is time: a healthy motor's high current is brief, a stalled motor's is sustained. An inverse curve, fast at high overload, slow at low overload, is exactly what lets a five to seven times FLC surge pass unhindered while the same current sustained for tens of seconds still trips the motor out.</div></div>

  <div class="n-h2">Trip class</div>
  <p class="n-p">Overload relays are rated by <strong>trip class</strong>, a number defined in IEC 60947-4-1 that states the maximum time in seconds the relay is allowed to take to trip when carrying 7.2 times its current setting from a cold start. Class 10 relays trip within roughly 4 to 10 seconds under that test, Class 20 within roughly 6 to 20 seconds, and Class 30 within roughly 9 to 30 seconds.</p>
  <table class="n-table">
    <tr><th>Trip class</th><th>Approx. trip time at 7.2 × setting, cold</th><th>Typical application</th></tr>
    <tr><td>Class 10</td><td>4 to 10 s</td><td>Fast starting loads, most general purpose motors, pumps and fans starting light</td></tr>
    <tr><td>Class 20</td><td>6 to 20 s</td><td>Loads with a longer run-up, higher inertia</td></tr>
    <tr><td>Class 30</td><td>9 to 30 s</td><td>Heavy starting duties, large inertia loads, some compressor and centrifuge drives</td></tr>
  </table>
  <p class="n-p">The class chosen has to match the motor's actual starting time. Fit a Class 10 relay to a machine needing 15 seconds to accelerate and it nuisance-trips on every start; fit a Class 30 relay to a small fan motor and it lets a genuinely stalled rotor cook far longer than the insulation can tolerate before it finally trips.</p>
