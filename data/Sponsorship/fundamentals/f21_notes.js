...
<div class="note-doc">
  ...
  <table class="n-table">
    ...
    <tr><td><strong>Power supply</strong></td><td>Converts the incoming supply, typically 24V DC or mains AC, to the regulated low voltage DC the CPU and internal electronics need. Often has its own back-up or is fed from an uninterruptible source, since losing the PLC supply loses control of everything downstream of it.</td></tr>
  ...
  <p class="n-p"><strong>Typical signal type:</strong> on marine and industrial control panels this is almost always <strong>24V DC</strong>. The field device either applies 24V to the input terminal, read as logic 1, or does not, read as logic 0.</p>
  ...
  <p class="n-p"><strong>Typical signal types:</strong> the two industry-standard conventions are <strong>4-20mA current loop</strong> and <strong>0-10V</strong> voltage signal. On ships, 4-20mA current loop dominates for field transmitters over any real cable run, because a current signal is unaffected by voltage drop or resistance along the cable, so the reading at the PLC end is accurate however long or resistive the run is; a voltage signal, by contrast, is directly reduced by cable resistance over distance.</p>
  <p class="n-p"><strong>Why 4mA and not 0mA at the bottom of range:</strong> this is called a <strong>live zero</strong> and it is a deliberate design choice, not an arbitrary number. With a 4-20mA scale, 0mA is a state the loop should never produce while healthy, so a reading of 0mA unambiguously means a broken wire, a disconnected transmitter, or a power failure, not "the process variable happens to be zero." A 0-20mA scheme cannot make that distinction: 0mA could mean either a genuine zero reading or a dead loop, and there is no way to tell them apart. The 4mA floor also conveniently doubles as the standing current that powers a loop-powered two-wire transmitter's own electronics, so the same two wires carry both power to the transmitter and the signal back.</p>
  ...
  An input that reads a signal which can only be one of two states, on or off, typically 24V DC present or absent. A level switch or a pushbutton are the standard shipboard examples. This is asked as a separate question from analog input in the real exam, so both examples need to be ready independently.</p>
