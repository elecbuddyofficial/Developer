...
<div class="note-doc">
  ...
  <div class="n-crit"><span class="icon">🔴</span><div class="body"><strong>Why laminating the core specifically reduces eddy current loss.</strong> Laminating does little to hysteresis loss, a property of the steel grade, not its shape. What it attacks is eddy current: instead of one solid block, where eddy currents form large loops across the whole cross section, the core is built from many thin, mutually insulated sheets, confining each eddy current to a small loop within one lamination. Eddy current loss varies with the <span class="n-val">square of lamination thickness</span>, so halving the thickness cuts the loss to roughly a quarter, not just a half. Typical laminations are <span class="n-val">0.25 to 0.5 mm</span> thick, a trade between minimising eddy loss and practical manufacturing.</div></div>
  <table class="n-table">
    <tr><th>&nbsp;</th><th>Hysteresis loss</th><th>Eddy current loss</th></tr>
    <tr><td>Cause</td><td>Domains resisting realignment as the field reverses</td><td>Circulating currents induced in the core steel itself</td></tr>
    <tr><td>Energy goes as</td><td>Area enclosed by the B-H loop, each cycle</td><td>I&sup2;R heating within the core</td></tr>
    <tr><td>Reduced by</td><td>Better steel grade, silicon steel with a narrow loop</td><td><span class="n-val">Laminating</span> the core</td></tr>
    <tr><td>Does laminating help?</td><td>Barely, it is a property of the steel not its shape</td><td>Yes, this is exactly what laminating attacks</td></tr>
  </table>
  ...
  <p class="n-p">Worked example: iron loss <span class="n-val">800 W</span>, full load copper loss 1250 W. Maximum efficiency occurs at x = √(800/1250) = √0.64 = <span class="n-val">0.8, i.e. 80 percent of full load</span>. At that loading, copper loss = 0.8² × 1250 = 800 W, exactly equal to the iron loss, confirming the condition.</p>
  ...
  <p class="n-p">Worked comparison: a load drawing <span class="n-val">100 kW</span> at unity power factor needs a certain current. The same 100 kW at 0.7 power factor needs roughly 1/0.7, about <span class="n-val">1.43 times</span> that current, and the I²R loss rises to roughly 1.43², about <span class="n-val">2 times</span>, for exactly the same delivered real power. Nothing useful was gained for that extra loss.</p>
