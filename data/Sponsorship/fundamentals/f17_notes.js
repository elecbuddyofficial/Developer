...
Reverse bias: depletion layer widens, current is blocked (a small leakage current only)</div>
  ...
  <p class="n-p">A discharge lamp, fluorescent or similar, run directly from an AC mains supply does not glow with constant brightness. The arc within the tube is re-struck and effectively re-ignited on every half-cycle of the supply as the current passes through zero and reverses. On a 50 Hz supply, that means the light output fluctuates at <strong>100 Hz</strong> (two flashes per cycle, one on each half-cycle); on a 60 Hz supply, at <strong>120 Hz</strong>. This flicker is far too fast for the human eye to perceive directly under normal viewing, which is exactly why the hazard is easy to miss.</p>
  ...
  <p class="n-p">Pressing a button on the handset causes a small microcontroller to generate a coded pulse pattern, a unique sequence representing that particular command, and use it to switch an infrared LED on and off very rapidly. Rather than simply pulsing the LED on and off at the slow rate needed to encode the command itself, the LED is switched on and off rapidly at a <strong>carrier frequency</strong>, commonly around <strong>38 kHz</strong>, only during the "on" portions of the coded command. So the actual transmitted signal is a burst of 38 kHz pulses, gated on and off in the pattern that represents the button pressed.</p>
  ...
  Diffusion of carriers across the junction leaves a carrier-free depletion layer with a small internal barrier voltage, about 0.7 V for silicon. Forward bias narrows the depletion layer and lets current flow easily above that barrier voltage. Reverse bias widens it and blocks current, aside from a small leakage current.</p>
