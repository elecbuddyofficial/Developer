...
<div class="note-doc">
  ...
  • <strong>Protection Response:</strong> The <strong>Reverse Power Relay</strong> of the idle machine should detect power flowing <em>into</em> it instead of <em>out</em> of it and trip its ACB within 3–5 seconds. The pick-up is set from the prime mover, not picked at random: a dead diesel still absorbs real power because it is being turned against its own compression, so it needs 8–15% of rated kW before the relay will call it motoring. A turbine freewheels and absorbs almost nothing, so 2–3% is enough. Set a diesel machine at turbine sensitivity and it will trip every time you synchronise. If the reverse power relay fails to trip, the running generator's <strong>Overcurrent Relay (OCR)</strong> will trip, causing a total ship <strong>blackout</strong>.</div></div>
  ...
  <table class="n-table">
    ...
    <tr><td>Reverse Power Relay</td><td class="hl">Set by prime mover: <span class="n-val">8–15%</span> rated kW for a <strong>diesel</strong>, <span class="n-val">2–3%</span> for a <strong>turbine</strong>, with a <span class="n-val">3–5 s</span> delay</td><td>VCB racked out: inject simulated reverse phase current and voltage</td></tr>
